/*
╭━━━⊰ 𝗕𝗮𝗶𝗹𝗲𝘆𝘀 𝗠𝗼𝗱𝗶𝗳𝗶𝗲𝗱 ⊱━━━╮
┃ 👨‍💻 Dibuat oleh: Kenz • Coding
┃ 🔗 GitHub: https://github.com/Kenz472/baileys 
┃ 💬 Grup Official: 
┃     https://chat.whatsapp.com/HlyYOczlYcn9JipRVchJxE 
╰━━━━━━━━━━━━━━━━━━━━━━╯
*/
"use strict";
        ...
        await authState.keys.transaction(async () => {
            ...
            if (participant) {
                if ((0, WABinary_1.isJidGroup)(destinationJid)) {
                    owns.attrs.to = destinationJid;
                    owns.attrs.participant = participant.jid;
                }
                else if ((0, WABinary_1.areJidsSameUser)(participant.jid, meId)) {
                    owns.attrs.to = participant.jid;
                    owns.attrs.recipient = destinationJid;
                }
                else {
                    owns.attrs.to = participant.jid;
                }
            ...
            if (!isNewsletter && buttonType) {
                ...
                if (filteredNode) {
                    ...
                    owns.content.push(...additionalNodes);
                }
                else {
                    owns.content.push(...content);
                }
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && content.album && Array.isArray(content.album)) {
                ...
                if (caption && !album[0].caption) album[0].caption = caption;

                const albumMsg = await Utils_1.generateWAMessageFromContent(jid, {
                    albumMessage: {
                        caption: caption || '',
                        expectedImageCount: album.filter(m => m.image).length,
                        expectedVideoCount: album.filter(m => m.video).length
                    }
                }, { userJid, quoted, ...options });
                ...
                for (const [idx, media] of album.entries()) {
                    let msg;
                    if (media.image) {
            ...
            if (messageType) {
                switch (messageType) {
                    case 'PAYMENT':
                        const paymentContent = await ows.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await ows.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await ows.handleInteractive(content, jid, quoted);
                    ...
                    case 'EVENT':
                        return await ows.handleEvent(content, jid, quoted);

                    case 'POLL_RESULT':
                        return await ows.handlePollResult(content, jid, quoted);
                }
            }
